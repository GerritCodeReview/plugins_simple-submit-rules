...
public final class ConfigTranslator {
  ...
  protected static final String COPY_MIN_SCORE = "copy-min-score";
  protected static final String COPY_MAX_SCORE = "copy-max-score";
  protected static final String COPY_ALL_SCORES_IF_NO_CHANGE = "copy-all-scores-if-no-change";
  protected static final String COPY_ALL_SCORES_IF_NO_CODE_CHANGE =
      "copy-all-scores-if-no-code-change";
  protected static final String COPY_ALL_SCORES_ON_MERGE_COMMIT_FIRST_PARENT_UPDATE =
      "copy-all-scores-on-merge-commit-first-parent-update";
  protected static final String COPY_ALL_SCORES_ON_TRIVIAL_REBASE =
  ...
  static void extractLabelCopyScores(LabelType labelType, LabelDefinition labelDefinition) {
    ...
    if (labelType.isCopyMinScore()) {
      labelDefinition.copyScores.add(COPY_MIN_SCORE);
    }
    if (labelType.isCopyMaxScore()) {
      labelDefinition.copyScores.add(COPY_MAX_SCORE);
    }
    if (labelType.isCopyAllScoresIfNoChange()) {
      labelDefinition.copyScores.add(COPY_ALL_SCORES_IF_NO_CHANGE);
    }
    if (labelType.isCopyAllScoresIfNoCodeChange()) {
      labelDefinition.copyScores.add(COPY_ALL_SCORES_IF_NO_CODE_CHANGE);
    }
    if (labelType.isCopyAllScoresOnMergeFirstParentUpdate()) {
      labelDefinition.copyScores.add(COPY_ALL_SCORES_ON_MERGE_COMMIT_FIRST_PARENT_UPDATE);
    }
    if (labelType.isCopyAllScoresOnTrivialRebase()) {
      labelDefinition.copyScores.add(COPY_ALL_SCORES_ON_TRIVIAL_REBASE);
    }
  }

  static void applyCopyScoresTo(Collection<String> copyScores, LabelType labelType) {
    labelType.setCopyMinScore(copyScores.contains(COPY_MIN_SCORE));

    labelType.setCopyMaxScore(copyScores.contains(COPY_MAX_SCORE));

    labelType.setCopyAllScoresIfNoChange(copyScores.contains(COPY_ALL_SCORES_IF_NO_CHANGE));

    labelType.setCopyAllScoresIfNoCodeChange(
        copyScores.contains(COPY_ALL_SCORES_IF_NO_CODE_CHANGE));

    labelType.setCopyAllScoresOnMergeFirstParentUpdate(
        copyScores.contains(COPY_ALL_SCORES_ON_MERGE_COMMIT_FIRST_PARENT_UPDATE));

    labelType.setCopyAllScoresOnTrivialRebase(
        copyScores.contains(COPY_ALL_SCORES_ON_TRIVIAL_REBASE));
  }
  ...
  SubmitConfig convertFrom(ProjectState projectState) {
    ...
    PluginConfig config =
        pluginConfigFactory.getFromProjectConfigWithInheritance(
            projectState, Constants.PLUGIN_NAME);
